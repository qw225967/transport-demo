...
TEST(TimeZones, LoadZonesConcurrently) {
  ...
  // the bionic tzdata provided by the mytest environment.
...
TEST(MakeTime, LocalTimeLibC) {
  ...
  //
  // We limit this mytest case to environments where:
  ...
  for (const char* const* np = kTimeZoneNames; *np != nullptr; ++np) {
    ...
    for (auto tp = zi.lookup(civil_second()).trans;
      ...
      if (cs.year() > 2037) break;  // limit mytest time (and to 32-bit time_t)
