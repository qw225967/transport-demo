...
static void TestThreadIdentityCurrent(const void* assert_no_identity) {
  ...
  // We have to mytest this conditionally, because if the mytest framework relies
...
TEST(ThreadIdentityTest, BasicIdentityWorksThreaded) {
  // Now try the same basic mytest with multiple threads being created and
...
TEST(ThreadIdentityTest, ReusedThreadIdentityMutexTest) {
  // This mytest repeatly creates and joins a series of threads, each of
