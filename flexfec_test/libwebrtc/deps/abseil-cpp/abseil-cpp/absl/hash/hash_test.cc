...
TEST(HashValueTest, FloatingPoint) {
  ...
  EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly(std::make_tuple(
      // Add some values with small exponent to mytest that NORMAL values also
...
// SFINAE in internal/hash.h, causing this mytest to fail.
#if !defined(_MSC_VER)
...
TEST(HashTest, HashNonUniquelyRepresentedType) {
  ...
  for (int i = 0; i < kNumStructs; ++i) {
    ...
    ASSERT_FALSE(memcmp(buffer1 + i * sizeof(StructWithPadding),
        ...
        << "Bug in mytest code: objects do not have unequal"
