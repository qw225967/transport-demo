...
TEST(ParseFloat, LargeDecimalMantissa) {
  ...
  // truncate additional digits.  We need to mytest that:
...
TEST(ParseFloat, LargeHexadecimalMantissa) {
  ...
  // additional digits as sticky,  We need to mytest that:
  ...
  // mytest sticky digit behavior.  The low bit should be set iff any dropped
  // digit is nonzero.
