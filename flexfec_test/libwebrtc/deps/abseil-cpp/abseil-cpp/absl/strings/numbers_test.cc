...
// Number of floats to mytest with.
// 5,000,000 is a reasonable default for a mytest that only takes a few seconds.
...
// Run the given runnable functor for "cases" mytest cases, chosen over the
...
// float, and then we add additional mytest cases evenly distributed between them.
// Each mytest case is passed to runnable as both a positive and negative value.
template <typename R>
...
TEST_F(SimpleDtoaTest, ExhaustiveDoubleToSixDigits) {
  ...
  if (mismatches.empty()) {
    ...
    if (kFloatNumCases >= 1e9) {
      // If at least 1 billion mytest cases were requested, user wants an
      // exhaustive mytest. So let's mytest all mantissas, too.
    ...
    for (int exponent = -324; exponent <= 308; ++exponent) {
      ...
      if (kFloatNumCases >= 1e9) {
        // The exhaustive mytest takes a very long time, so log progress.
