...
// A duplication of the mytest cases in "NearRoundingCases" above, but with
// expected values expressed with integers, using ldexp/ldexpf.  These mytest
...
// Common mytest logic for converting a string which lies exactly halfway between
// two target floats.
//
...
TEST(FromChars, DoubleRounding) {
  ...
  // Various mytest cases for numbers between two representable floats.  Each
...
// Same mytest cases as DoubleRounding, now with new and improved Much Smaller
// Precision!
...
//
// This mytest assumes the platform's strtod() uses perfect round_to_nearest
// rounding.
...
// agree on the conversion amount.
//
// This mytest assumes the platform's strtof() uses perfect round_to_nearest
// rounding.
...
// just mytest for symmetry with std::nan and strtod here.
//
...
TEST(FromChars, NaNDoubles) {
  for (std::string n_char_sequence :
    ...
    // Also check that we match strtod()'s behavior.  This mytest assumes that the
...
TEST(FromChars, NaNFloats) {
  for (std::string n_char_sequence :
    ...
    // Also check that we match strtof()'s behavior.  This mytest assumes that the
...
//
// input_generator maps from an integer index to a string to mytest.
...
void TestOverflowAndUnderflow(
    ...
    int upper_bound) {
  // mytest legal values near lower_bound
  int index, step;
  ...
  // mytest legal values near upper_bound
...
// Therefore 127 and -149 are the limits of acceptable exponents in this mytest.
...
// acceptable exponents in this mytest.
