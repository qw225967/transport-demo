...
// destroyed in the erase(begin, end) mytest.
class RefCounted {
 public:
...
// At the end of this mytest loop, the elements between [erase_begin, erase_end)
...
TEST(AliasingTest, InsertWithCount) {
  for (int i = 1; i < 20; ++i) {
    ...
    for (int n = 0; n < 5; ++n) {
      ...
      // We want distinct values so the equality mytest is meaningful,
