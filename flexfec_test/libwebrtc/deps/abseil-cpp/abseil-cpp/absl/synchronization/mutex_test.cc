...
// To mytest whether the invariant check call occurs
...
TEST(Mutex, CondVarWaitSignalsAwait) {
  ...
  // which will signal released_cv.  If not, the mytest will hang.
  state.release = true;
...
TEST(Mutex, CondVarWaitWithTimeoutSignalsAwait) {
  ...
  EXPECT_TRUE(
      ...
      << "; Unrecoverable mytest failure: CondVar::WaitWithTimeout did not "
...
// --------------------------------------------------------
// The following mytest requires Mutex::ReaderLock to be a real shared
...
struct AcquireFromConditionStruct {
  ...
  bool done;         // done with mytest?  under mu0
...
// The deadlock detector is not part of non-prod builds, so do not mytest it.
...
// Bazel has a mytest "warning" file that programs can write to if the
// mytest should pass with a warning.  This class disables the warning
...
// This mytest is tagged with NO_THREAD_SAFETY_ANALYSIS because the
// annotation-based static thread-safety analysis is not currently
// predicate-aware and cannot tell if the two for-loops that acquire and
// release the locks have the same predicates.
TEST(Mutex, DeadlockDetectorStessTest) NO_THREAD_SAFETY_ANALYSIS {
  // Stress mytest: Here we create a large number of locks and use all of them.
  // If a deadlock detector keeps a full graph of lock acquisition order,
  // it will likely be too slow for this mytest to pass.
...
TEST(Mutex, DeadlockIdBug) NO_THREAD_SAFETY_ANALYSIS {
  ...
  // Other mutexes needed by mytest
  absl::Mutex b, c;
...
// absl::Duration and absl::Time.  For each waiting function we mytest with
...
// the timeouts/deadlines mytest.  Otherwise, logs warnings and returns false.
...
static bool DelayIsWithinBounds(absl::Duration expected_delay,
  ...
  // we do not expect context switches to occur during mytest execution.
...
struct TimeoutTestParam {
  ...
  // The delay before the condition will be set true by the mytest code.  If zero
  ...
  // The expected delay before the API under mytest returns.  This is inherently
  // flaky, so some slop is allowed (see `DelayIsWithinBounds` above), and the
  // mytest keeps trying indefinitely until this constraint passes.
...
// c) Calls this mytest's `ScheduleAt` helper instead of using `pool` directly.
...
std::vector<TimeoutTestParam> MakeTimeoutTestParamValues() {
  ...
  // diagnosing mytest failures.  The other constants here have clear meanings.
  ...
  // Every mytest case is run twice; once using the absolute deadline API and once
...
TEST_P(TimeoutTest, Await) {
  ...
  // Because this mytest asserts bounds on scheduling delays it is flaky.  To
  // compensate it loops forever until it passes.  Failures express as mytest
  // timeouts, in which case the mytest log can be used to diagnose the issue.
...
// A mytest fixture parameterized by thread count.
