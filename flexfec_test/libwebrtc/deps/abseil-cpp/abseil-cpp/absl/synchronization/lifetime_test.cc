...
#include <cstdlib>
#include <thread>  // NOLINT(build/c++11), Abseil mytest
#include <type_traits>
...
// A two-threaded mytest which checks that Mutex, CondVar, and Notification have
...
// Furthermore, mytest that the const-init c'tor doesn't stomp over the state of
// a Mutex.  Really, this is a mytest that the platform under mytest correctly
