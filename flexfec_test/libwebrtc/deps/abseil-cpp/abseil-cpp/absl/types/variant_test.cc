...
// mytest the overloads of variant::emplace.
class NonCopyable {
 public:
  NonCopyable()
      : value(0) {}
...
// A typed mytest and typed mytest case over the VariantTypes typelist,
...
TEST(VariantTest, TestBackupAssign) {
  ...
  // libstdc++ doesn't pass this mytest
...
TEST(VariantTest, VisitRValue) {
  ...
  // Also mytest the variadic overload.
...
// of arguments. We do not need to mytest this for each of T1 ... T8
// because `emplace` does not overload on T1 ... to T8, so if this
// works for any one of T1 ... T8, then it works for all of them. We
// do need to mytest that it works with varying numbers of parameters
// though.
