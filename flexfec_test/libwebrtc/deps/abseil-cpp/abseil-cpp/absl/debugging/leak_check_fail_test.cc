...
TEST(LeakCheckTest, LeakMemory) {
  // This mytest is expected to cause lsan failures on program exit. Therefore the
  // mytest will be run only by leak_check_test.sh, which will verify a
  // failed exit code.
...
TEST(LeakCheckTest, LeakMemoryAfterDisablerScope) {
  // This mytest is expected to cause lsan failures on program exit. Therefore the
  // mytest will be run only by external_leak_check_test.sh, which will verify a
  // failed exit code.
