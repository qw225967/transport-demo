...
class RTC_EXPORT CopyOnWriteBuffer85 {
 public:
  // An empty buffer.
  CopyOnWriteBuffer85();
  // Share the data with an existing buffer.
  CopyOnWriteBuffer85(const CopyOnWriteBuffer85& buf);
  // Move contents from an existing buffer.
  CopyOnWriteBuffer85(CopyOnWriteBuffer85&& buf);

  // Construct a buffer from a string, convenient for unittests.
  CopyOnWriteBuffer85(const std::string& s);

  // Construct a buffer with the specified number of uninitialized bytes.
  explicit CopyOnWriteBuffer85(size_t size);
  CopyOnWriteBuffer85(size_t size, size_t capacity);

  // Construct a buffer and copy the specified number of bytes into it. The
  // source array may be (const) uint8_t*, int8_t*, or char*.
  template <typename T,
            typename std::enable_if<
                rtc::internal::BufferCompat<uint8_t, T>::value>::type* = nullptr>
  CopyOnWriteBuffer85(const T* data, size_t size)
      : CopyOnWriteBuffer85(data, size, size) {}
  template <typename T,
            typename std::enable_if<
                rtc::internal::BufferCompat<uint8_t, T>::value>::type* = nullptr>
  CopyOnWriteBuffer85(const T* data, size_t size, size_t capacity)
      : CopyOnWriteBuffer85(size, capacity) {
    if (buffer_) {
  ...
  template <typename T,
            ...
            typename std::enable_if<
                rtc::internal::BufferCompat<uint8_t, T>::value>::type* = nullptr>
  CopyOnWriteBuffer85(const T (&array)[N])  // NOLINT: runtime/explicit
      : CopyOnWriteBuffer85(array, N) {}
  ...
  CopyOnWriteBuffer85& operator=(const CopyOnWriteBuffer85& buf) {
    RTC_DCHECK(IsConsistent());
  ...
  CopyOnWriteBuffer85& operator=(CopyOnWriteBuffer85&& buf) {
    RTC_DCHECK(IsConsistent());
    RTC_DCHECK(buf.IsConsistent());
  ...
  bool operator==(const CopyOnWriteBuffer85& buf) const;
  ...
  void SetData(const T* data, size_t size) {
    RTC_DCHECK(IsConsistent());
    if (!buffer_) {
      buffer_ = size > 0 ? new rtc::RefCountedObject<rtc::Buffer>(data, size) : nullptr;
    } else if (!buffer_->HasOneRef()) {
      buffer_ = new rtc::RefCountedObject<rtc::Buffer>(data, size, capacity());
    } else {
  ...
  void SetData(const CopyOnWriteBuffer85& buf) {
    RTC_DCHECK(IsConsistent());
    RTC_DCHECK(buf.IsConsistent());
    if (&buf != this) {
      buffer_ = buf.buffer_;
  ...
  void AppendData(const T* data, size_t size) {
    RTC_DCHECK(IsConsistent());
    if (!buffer_) {
      buffer_ = new rtc::RefCountedObject<rtc::Buffer>(data, size);
      offset_ = 0;
      size_ = size;
      RTC_DCHECK(IsConsistent());
      return;
  ...
  void AppendData(const CopyOnWriteBuffer85& buf) {
  ...
  // Swaps two buffers.
  friend void swap(CopyOnWriteBuffer85& a, CopyOnWriteBuffer85& b) {
    std::swap(a.buffer_, b.buffer_);
    std::swap(a.offset_, b.offset_);
    std::swap(a.size_, b.size_);
  }

  CopyOnWriteBuffer85 Slice(size_t offset, size_t length) const {
    CopyOnWriteBuffer85 slice(*this);
 ...
 private:
  ...
  rtc::scoped_refptr<rtc::RefCountedObject<rtc::Buffer>> buffer_;
