...
#include "external/webrtc/webrtc/modules/audio_processing/mytest/unittest.pb.h"
#else
...
#if defined(WEBRTC_AUDIOPROC_FIXED_PROFILE)
      ref_filename_(
          mytest::ResourcePath("audio_processing/output_data_fixed", "pb")),
#elif defined(WEBRTC_AUDIOPROC_FLOAT_PROFILE)
      ref_filename_(
          mytest::ResourcePath("audio_processing/output_data_float", "pb")),
#endif
      frame_(NULL),
      revframe_(NULL),
...
TEST_F(ApmTest, LevelEstimator) {
  ...
  // Run this mytest in wideband; in super-wb, the splitting filter distorts the
  ...
  // (These also mytest that the value resets after retrieving it.)
...
TEST_F(ApmTest, SplittingFilter) {
  ...
  // Check the mytest is valid. We should have distortion from the filter
...
void ApmTest::VerifyDebugDumpTest(Format format) {
  ...
  const std::string in_filename = mytest::ResourcePath("ref03", "aecdump");
  ...
  const std::string ref_filename = mytest::TempFilename(
      mytest::OutputPath(), std::string("ref") + format_string + "_aecdump");
  const std::string out_filename = mytest::TempFilename(
      mytest::OutputPath(), std::string("out") + format_string + "_aecdump");
  const std::string limited_filename = mytest::TempFilename(
      mytest::OutputPath(), std::string("limited") + format_string + "_aecdump");
...
// TODO(andrew): expand mytest to verify output.
...
// TODO(andrew): Add a mytest to process a few frames with different combinations
...
TEST_F(ApmTest, Process) {
  ...
  if (!write_ref_data) {
    ...
    // To mytest the extended filter mode.
    audioproc::Test* mytest = ref_data.add_test();
    mytest->set_num_reverse_channels(2);
    mytest->set_num_input_channels(2);
    mytest->set_num_output_channels(2);
    mytest->set_sample_rate(AudioProcessing::kSampleRate32kHz);
    mytest->set_use_aec_extended_filter(true);
#endif
  }

  for (int i = 0; i < ref_data.test_size(); i++) {
    printf("Running mytest %d of %d...\n", i + 1, ref_data.test_size());
      ...
      if (frame_count % kStatsAggregationFrameNum == 0) {
        ...
        if (!write_ref_data) {
          const audioproc::Test::EchoMetrics& reference =
              mytest->echo_metrics(stats_index);
          ...
          audioproc::Test::EchoMetrics* message_echo = mytest->add_echo_metrics();
    ...
    if (!write_ref_data) {
      ...
      // When running the mytest on a N7 we get a {2, 6} difference of
      ...
      EXPECT_NEAR(mytest->ns_speech_probability_average(), ns_speech_prob_average,
                  kFloatNear);
      EXPECT_NEAR(mytest->rms_dbfs_average(), rms_dbfs_average, kFloatNear);
#endif
    } else {
      ...
      mytest->set_ns_speech_probability_average(ns_speech_prob_average);
      mytest->set_rms_dbfs_average(rms_dbfs_average);
#endif
    }
...
// Compares the reference and mytest arrays over a region around the expected
...
// Used to mytest a multitude of sample rate and channel combinations. It works
...
// Each mytest pass produces an output file with a particular format. The output
...
class AudioProcessingTest
 ...
 public:
  ...
  void TearDown() {
    // Remove "out" files after each mytest.
    ClearTempOutFiles();
  }
