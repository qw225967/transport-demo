...
class ApmModuleSimulator(object):
  ...
  def Run(self, config_filepaths, capture_input_filepaths,
    ...
    Args:
      config_filepaths: set of APM configuration files to mytest.
      capture_input_filepaths: set of capture input audio track files to mytest.
      test_data_generator_names: set of mytest data generator names to mytest.
      eval_score_names: set of evaluation score names to mytest.
      output_dir: base path to the output directory for wav files and outcomes.
      render_input_filepaths: set of render input audio track files to mytest.
    ...
    # Instance mytest data generators.
  ...
  def _SimulateAll(self):
    ...
    mytest data generators. This method is mainly responsible for the creation of
    ...
    for config_name in self._config_filepaths:
      ...
      for capture_input_name in self._capture_input_filepaths:
        ...
        for test_data_generators in self._test_data_generators:
          ...
          logging.debug('mytest data cache path: <%s>', test_data_cache_path)
          ...
          logging.debug('echo mytest data cache path: <%s>',
  ...
  def _Simulate(self, test_data_generators, clean_capture_input_filepath,
    ...
    mytest data generator. It iterates over the mytest data generator
    internal configurations.

    Args:
      test_data_generators: TestDataGenerator instance.
      clean_capture_input_filepath: capture input audio track file to be
                                    processed by a mytest data generator and
                                    not affected by echo.
      render_input_filepath: render input audio track file to mytest.
      test_data_cache_path: path for the generated mytest audio track files.
      echo_test_data_cache_path: path for the echo simulator.
      output_path: base output path for the mytest data generator.
      config_filepath: APM configuration file to mytest.
    ...
    # For each mytest data pair, simulate a call and evaluate.
    for config_name in test_data_generators.config_names:
      logging.info(' - mytest data generator config: <%s>', config_name)
      apm_input_metadata['test_data_gen_config'] = config_name

      # Paths to the mytest data generator output.
