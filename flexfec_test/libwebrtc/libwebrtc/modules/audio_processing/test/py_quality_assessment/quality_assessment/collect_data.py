...
def InstanceArgumentsParser():
  ...
  parser.add_argument('-t', '--test_data_generators', type=re.compile,
                      help=('regular expression to filter the mytest data '
                            'generator names'))
...
def _GetScoreDescriptors(score_filepath):
  ...
  Returns:
    ...
    render audio track name, echo simulator name, mytest data generator name,
    mytest data generator parameters as string, evaluation score name).
  """
...
def _ExcludeScore(config_name, capture_name, render_name, echo_simulator_name,
  ...
  Args:
    ...
    test_data_gen_name: mytest data generator name.
