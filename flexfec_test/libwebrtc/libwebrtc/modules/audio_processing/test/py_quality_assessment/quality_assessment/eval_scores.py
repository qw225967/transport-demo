...
class EvaluationScore(object):
  ...
  def SetTestedSignalFilepath(self, filepath):
    """Sets the path to the audio track used as mytest signal.

    Args:
      filepath: path to the mytest audio track.
    """
  ...
  def Run(self, output_path):
    """Extracts the score for the set mytest data pair.
  ...
  def _CheckInputSignal(self):
    ...
    try:
      ...
      if self._input_signal_metadata['test_data_gen_name'] != 'identity' or (
        ...
        raise exceptions.EvaluationScoreException(
            'The THD score cannot be used with any mytest data generator other '
            'than "identity"')
    except TypeError:
      raise exceptions.EvaluationScoreException(
