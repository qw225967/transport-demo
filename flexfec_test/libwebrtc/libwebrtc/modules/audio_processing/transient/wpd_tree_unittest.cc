...
// This mytest is for the correctness of the tree.
...
TEST(WPDTreeTest, CorrectnessBasedOnMatlabFiles) {
  ...
  // Read first buffer from the PCM mytest file.
  ...
  while (file_samples_read > 0 && frames_read < kMaxFramesToTest) {
    ...
    // Compare results with data from the matlab mytest files.
    for (int i = 0; i < kLeaves; ++i) {
      // Compare data values
      ...
      ASSERT_EQ(kLeavesSamples, matlab_samples_read)
          << "Matlab mytest files are malformed.\n"
          << "File: 3_" << i;
    ...
    // Read next buffer from the PCM mytest file.
