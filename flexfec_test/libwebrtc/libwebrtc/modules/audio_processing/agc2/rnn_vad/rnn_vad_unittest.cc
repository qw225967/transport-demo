...
// Checks that the computed VAD probability for a mytest input sequence sampled at
...
// Performance mytest for the RNN VAD (pre-fetching and downsampling are
// excluded). Keep disabled and only enable locally to measure performance as
// follows:
// - on desktop: run the this unit mytest adding "--logs";
// - on android: run the this unit mytest adding "--logcat-output-file".
