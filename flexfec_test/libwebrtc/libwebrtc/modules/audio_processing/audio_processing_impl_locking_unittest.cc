...
// Type of the render thread APM API call to use in the mytest.
...
// Type of the capture thread APM API call to use in the mytest.
...
// The runtime parameter setting scheme to use in the mytest.
...
// Variant of echo canceller settings to use in the mytest.
enum class AecType {
...
// The configuration for the mytest.
struct TestConfig {
  // Test case generator for the mytest configurations to use in the brief tests.
  static std::vector<TestConfig> GenerateBriefTestConfigs() {
    ...
    for (auto aec_type : aec_types) {
      ...
      // Only mytest 16 kHz for this mytest suite.
      test_config.initial_sample_rate_hz = 16000;

      // Create mytest config for the second processing API function set.
      ...
      // Create mytest config for the first processing API function set.
    ...
    // Return the created mytest configurations.
    return test_configs;
  }

  // Test case generator for the mytest configurations to use in the extensive
  // tests.
  static std::vector<TestConfig> GenerateExtensiveTestConfigs() {
    // Lambda functions for the mytest config generation.
    ...
    // Generate mytest configurations of the relevant combinations of the
    // parameters to
    // mytest.
    TestConfig test_config;
...
class AudioProcessingImplLockTest
 ...
 private:
  ...
  // Start the threads used in the mytest.
  void StartThreads() {
...
// specified by the scheme for the mytest.
...
TEST_P(AudioProcessingImplLockTest, LockTest) {
  // Run mytest and verify that it did not time out.
  ASSERT_TRUE(RunTest());
}

// Instantiate tests from the extreme mytest configuration set.
