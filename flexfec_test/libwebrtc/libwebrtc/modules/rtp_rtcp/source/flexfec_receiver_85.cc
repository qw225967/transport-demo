...
FlexfecReceiver::FlexfecReceiver(
    ...
    : FlexfecReceiver(webrtc::Clock::GetRealTimeClock(),
                      ssrc,
...
FlexfecReceiver::FlexfecReceiver(
    ...
    : ssrc_(ssrc),
      ...
      erasure_code_(
          webrtc85::ForwardErrorCorrection::CreateFlexfec(ssrc, protected_media_ssrc)),
...
void FlexfecReceiver::OnRtpPacket(const RtpPacketReceived& packet) {
  ...
  std::unique_ptr<webrtc85::ForwardErrorCorrection::ReceivedPacket> received_packet =
...
std::unique_ptr<webrtc85::ForwardErrorCorrection::ReceivedPacket>
FlexfecReceiver::AddReceivedPacket(const RtpPacketReceived& packet) {
  ...
  RTC_DCHECK_GE(packet.size(), webrtc::kRtpHeaderSize);

  // Demultiplex based on SSRC, and insert into erasure code decoder.
  std::unique_ptr<webrtc85::ForwardErrorCorrection::ReceivedPacket> received_packet(
      new webrtc85::ForwardErrorCorrection::ReceivedPacket());
...
//      RTC_LOG(LS_WARNING) << "Truncated FlexFEC packet, discarding.";
    ...
    received_packet->pkt = rtc::scoped_refptr<webrtc85::ForwardErrorCorrection::Packet>(
        new webrtc85::ForwardErrorCorrection::Packet());
...
void FlexfecReceiver::ProcessReceivedPacket(
  ...
  for (const auto& recovered_packet : recovered_packets_) {
    ...
    if (now_ms - last_recovered_packet_ms_ > kPacketLogIntervalMs) {
      uint32_t media_ssrc =
          webrtc85::ForwardErrorCorrection::ParseSsrc(recovered_packet->pkt->data.data());
