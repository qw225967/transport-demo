...
#include "modules/rtp_rtcp/source/flexfec_header_reader_writer_85.h"
...
bool FlexfecHeaderReader::ReadFecHeader(
    webrtc85::ForwardErrorCorrection::ReceivedFecPacket* fec_packet) const {
...
//           "not yet support this, thus discarding packet.";
  ...
  uint8_t ssrc_count = webrtc::ByteReader<uint8_t>::ReadBigEndian(&data[8]);
  if (ssrc_count != 1) {
//    RTC_LOG(LS_INFO)
...
//           "yet support this, thus discarding packet.";
  ...
  uint32_t protected_ssrc = webrtc::ByteReader<uint32_t>::ReadBigEndian(&data[12]);
  uint16_t seq_num_base = webrtc::ByteReader<uint16_t>::ReadBigEndian(&data[16]);
...
//    RTC_LOG(LS_WARNING) << "Discarding truncated FlexFEC packet.";
  ...
  uint16_t mask_part0 = webrtc::ByteReader<uint16_t>::ReadBigEndian(&packet_mask[0]);
  // Shift away K-bit 0, implicitly clearing the last bit.
  mask_part0 <<= 1;
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(&packet_mask[0], mask_part0);
  ...
  if (k_bit0) {
    ...
    packet_mask[1] |= bit15;
    uint32_t mask_part1 = webrtc::ByteReader<uint32_t>::ReadBigEndian(&packet_mask[2]);
    // Shift away K-bit 1 and bit 15, implicitly clearing the last two bits.
    mask_part1 <<= 2;
    webrtc::ByteWriter<uint32_t>::WriteBigEndian(&packet_mask[2], mask_part1);
    if (k_bit1) {
...
//            << "Discarding FlexFEC packet with malformed header.";
      ...
      uint64_t mask_part2 =
          webrtc::ByteReader<uint64_t>::ReadBigEndian(&packet_mask[6]);
      // Shift away K-bit 2, bit 46, and bit 47, implicitly clearing the last
      // three bits.
      mask_part2 <<= 3;
      webrtc::ByteWriter<uint64_t>::WriteBigEndian(&packet_mask[6], mask_part2);
    }
  }
...
size_t FlexfecHeaderWriter::MinPacketMaskSize(const uint8_t* packet_mask,
  ...
  } else if (packet_mask_size == webrtc::kUlpfecPacketMaskSizeLBitClear) {
  ...
  } else if (packet_mask_size == webrtc::kUlpfecPacketMaskSizeLBitSet &&
...
void FlexfecHeaderWriter::FinalizeFecHeader(
    ...
    size_t packet_mask_size,
    webrtc85::ForwardErrorCorrection::Packet* fec_packet) const {
  uint8_t* data = fec_packet->data.data();
  data[0] &= 0x7f;  // Clear R bit.
  data[0] &= 0xbf;  // Clear F bit.
  webrtc::ByteWriter<uint8_t>::WriteBigEndian(&data[8], kSsrcCount);
  webrtc::ByteWriter<uint32_t, 3>::WriteBigEndian(&data[9], kReservedBits);
  webrtc::ByteWriter<uint32_t>::WriteBigEndian(&data[12], media_ssrc);
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(&data[16], seq_num_base);
  ...
  if (packet_mask_size == webrtc::kUlpfecPacketMaskSizeLBitSet) {
    // The packet mask is 48 bits long.
    uint16_t tmp_mask_part0 =
        webrtc::ByteReader<uint16_t>::ReadBigEndian(&packet_mask[0]);
    uint32_t tmp_mask_part1 =
        webrtc::ByteReader<uint32_t>::ReadBigEndian(&packet_mask[2]);

    tmp_mask_part0 >>= 1;  // Shift, thus clearing K-bit 0.
    webrtc::ByteWriter<uint16_t>::WriteBigEndian(&written_packet_mask[0],
                                         tmp_mask_part0);
    tmp_mask_part1 >>= 2;  // Shift, thus clearing K-bit 1 and bit 15.
    webrtc::ByteWriter<uint32_t>::WriteBigEndian(&written_packet_mask[2],
                                         tmp_mask_part1);
