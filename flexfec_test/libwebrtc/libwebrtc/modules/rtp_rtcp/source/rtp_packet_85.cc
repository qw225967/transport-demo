...
std::vector<uint32_t> RtpPacket::Csrcs() const {
  ...
  for (size_t i = 0; i < num_csrc; ++i) {
    csrcs[i] =
        webrtc::ByteReader<uint32_t>::ReadBigEndian(&data()[kFixedHeaderSize + i * 4]);
  }
  return csrcs;
}
...
void RtpPacket::SetTimestamp(uint32_t timestamp) {
  ...
  webrtc::ByteWriter<uint32_t>::WriteBigEndian(WriteAt(4), timestamp);
}
...
void RtpPacket::ZeroMutableExtensions() {
  for (const ExtensionInfo& extension : extension_entries_) {
    switch (extensions_.GetType(extension.id)) {
      case webrtc::RTPExtensionType::kRtpExtensionNone: {
//        RTC_LOG(LS_WARNING) << "Unidentified extension in the packet.";
        break;
      }
      case webrtc::RTPExtensionType::kRtpExtensionVideoTiming: {
      ...
      case webrtc::RTPExtensionType::kRtpExtensionTransportSequenceNumber:
        case webrtc::RTPExtensionType::kRtpExtensionTransportSequenceNumber02:
          case webrtc::RTPExtensionType::kRtpExtensionTransmissionTimeOffset:
            case webrtc::RTPExtensionType::kRtpExtensionAbsoluteSendTime: {
        // Nullify whole extension, as it's filled in the pacer.
        memset(WriteAt(extension.offset), 0, extension.length);
        break;
      }
      case webrtc::RTPExtensionType::kRtpExtensionAudioLevel:
        case webrtc::RTPExtensionType::kRtpExtensionAbsoluteCaptureTime:
          case webrtc::RTPExtensionType::kRtpExtensionColorSpace:
            case webrtc::RTPExtensionType::kRtpExtensionGenericFrameDescriptor00:
              case webrtc::RTPExtensionType::kRtpExtensionGenericFrameDescriptor02:
                case webrtc::RTPExtensionType::kRtpExtensionMid:
                  case webrtc::RTPExtensionType::kRtpExtensionNumberOfExtensions:
                    case webrtc::RTPExtensionType::kRtpExtensionPlayoutDelay:
                      case webrtc::RTPExtensionType::kRtpExtensionRepairedRtpStreamId:
                        case webrtc::RTPExtensionType::kRtpExtensionRtpStreamId:
                          case webrtc::RTPExtensionType::kRtpExtensionVideoContentType:
                            case webrtc::RTPExtensionType::kRtpExtensionVideoRotation:
                              case webrtc::RTPExtensionType::kRtpExtensionInbandComfortNoise: {
...
void RtpPacket::SetCsrcs(rtc::ArrayView<const uint32_t> csrcs) {
  ...
  for (uint32_t csrc : csrcs) {
    webrtc::ByteWriter<uint32_t>::WriteBigEndian(WriteAt(offset), csrc);
    offset += 4;
  }
...
rtc::ArrayView<uint8_t> RtpPacket::AllocateRawExtension(int id, size_t length) {
  ...
  RTC_DCHECK_LE(length, webrtc::RtpExtension::kMaxValueSize);
...
//                      << " after padding was set.";
  ...
  const bool two_byte_header_required =
      id > webrtc::RtpExtension::kOneByteHeaderExtensionMaxId ||
      length > webrtc::RtpExtension::kOneByteHeaderExtensionMaxValueSize || length == 0;
  RTC_CHECK(!two_byte_header_required || extensions_.ExtmapAllowMixed());

  uint16_t profile_id;
  if (extensions_size_ > 0) {
    profile_id =
        webrtc::ByteReader<uint16_t>::ReadBigEndian(data() + extensions_offset - 4);
...
//        << "Extension cannot be registered: Not enough space left in buffer.";
  ...
  if (extensions_size_ == 0) {
    ...
    webrtc::ByteWriter<uint16_t>::WriteBigEndian(WriteAt(extensions_offset - 4),
                                         profile_id);
  }
...
void RtpPacket::PromoteToTwoByteHeaderExtension() {
  ...
  RTC_CHECK_EQ(kOneByteExtensionProfileId, webrtc::ByteReader<uint16_t>::ReadBigEndian(
...
uint16_t RtpPacket::SetExtensionLengthMaybeAddZeroPadding(
  ...
  uint16_t extensions_words = rtc::dchecked_cast<uint16_t>(
      ...
      webrtc::ByteWriter<uint16_t>::WriteBigEndian(WriteAt(extensions_offset - 2),
                                       extensions_words);
