...
constexpr size_t kFecLevel1HeaderSizeLBitSet = 2 + webrtc::kUlpfecPacketMaskSizeLBitSet;
...
bool UlpfecHeaderReader::ReadFecHeader(
  ...
  size_t packet_mask_size =
      l_bit ? webrtc::kUlpfecPacketMaskSizeLBitSet : webrtc::kUlpfecPacketMaskSizeLBitClear;
  fec_packet->fec_header_size = UlpfecHeaderSize(packet_mask_size);
  uint16_t seq_num_base = webrtc::ByteReader<uint16_t>::ReadBigEndian(&data[2]);
  ...
  fec_packet->protection_length =
      webrtc::ByteReader<uint16_t>::ReadBigEndian(&data[10]);
...
void UlpfecHeaderWriter::FinalizeFecHeader(
  ...
  bool l_bit = (packet_mask_size == webrtc::kUlpfecPacketMaskSizeLBitSet);
  if (l_bit) {
    data[0] |= 0x40;  // Set the L bit.
  } else {
    RTC_DCHECK_EQ(packet_mask_size, webrtc::kUlpfecPacketMaskSizeLBitClear);
  ...
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(&data[2], seq_num_base);
