...
//        << "Received FEC packet is protecting an unknown media SSRC; dropping.";
  ...
  std::cout << "fec seq:" << fec_packet->seq_num << ", mask_size:" << fec_packet->packet_mask_size << std::endl;
    ...
    for (uint16_t bit_idx = 0; bit_idx < 8; ++bit_idx) {
      if (packet_mask & (1 << (7 - bit_idx))) {
        std::unique_ptr<ProtectedPacket> protected_packet(
            new ProtectedPacket());
        std::cout << "fec protected seq:" << static_cast<uint16_t>(fec_packet->seq_num_base + (byte_idx << 3) + bit_idx) << ", packet_mask:" << uint16_t (packet_mask) << ", byte_idx:"  << byte_idx << ", packet_mask_offset:"<< fec_packet->packet_mask_offset << ", base_seq:" << fec_packet->seq_num_base << std::endl;
