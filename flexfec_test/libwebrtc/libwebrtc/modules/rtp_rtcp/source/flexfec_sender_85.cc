...
// RTP timestamp.
const int kMsToRtpTimestamp = webrtc::kVideoPayloadTypeFrequency / 1000;

// How often to log the generated FEC packets to the text log.
constexpr int64_t kPacketLogIntervalMs = 10000;

webrtc::RtpHeaderExtensionMap RegisterSupportedExtensions(
  ...
  for (const auto& extension : rtp_header_extensions) {
    if (extension.uri == webrtc85::TransportSequenceNumber::kUri) {
      map.Register<webrtc85::TransportSequenceNumber>(extension.id);
    } else if (extension.uri == webrtc85::AbsoluteSendTime::kUri) {
      map.Register<webrtc85::AbsoluteSendTime>(extension.id);
    } else if (extension.uri == webrtc85::TransmissionOffset::kUri) {
      map.Register<webrtc85::TransmissionOffset>(extension.id);
    } else if (extension.uri == webrtc85::RtpMid::kUri) {
      map.Register<webrtc85::RtpMid>(extension.id);
    } else {
//      RTC_LOG(LS_INFO)
...
webrtc::DataRate FlexfecSender::CurrentFecRate() const {
  rtc::CritScope cs(&crit_);
  return webrtc::DataRate::BitsPerSec(
      fec_bitrate_.Rate(clock_->TimeInMilliseconds()).value_or(0));
}

absl::optional<webrtc::RtpState> FlexfecSender::GetRtpState() {
  webrtc::RtpState rtp_state;
