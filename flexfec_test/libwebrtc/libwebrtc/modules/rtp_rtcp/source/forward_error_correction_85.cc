...
#include "modules/rtp_rtcp/source/ulpfec_header_reader_writer_85.h"
    ...
    while (media_packets_it != media_packets.end()) {
      ...
      if (packet_masks_[pkt_mask_idx] & (1 << (7 - media_pkt_idx))) {
        size_t media_payload_length =
            media_packet->data.size() - webrtc::kRtpHeaderSize;
        ...
        if (first_protected_packet) {
          ...
          // ULPFEC.)
          webrtc::ByteWriter<uint16_t>::WriteBigEndian(&data[2], media_payload_length);
          // Write timestamp recovery field.
          memcpy(&data[4], &media_packet_data[4], 4);
          // Write payload.
          if (media_payload_length > 0) {
            memcpy(&data[fec_header_size], &media_packet_data[webrtc::kRtpHeaderSize],
...
int ForwardErrorCorrection::InsertZerosInPacketMasks(
  ...
  size_t tmp_packet_mask_size =
      webrtc::internal::PacketMaskSize(total_missing_seq_nums + num_media_packets);
  ...
  webrtc::internal::CopyColumn(tmp_packet_masks_, tmp_packet_mask_size, packet_masks_,
  ...
  while (media_packets_it != media_packets.end()) {
    ...
    if (num_zeros_to_insert > 0) {
      webrtc::internal::InsertZeroColumns(num_zeros_to_insert, tmp_packet_masks_,
...
//           "for its own header.";
  ...
  std::min(size_t{IP_PACKET_SIZE - webrtc::kRtpHeaderSize},
...
//    RTC_LOG(LS_WARNING) << "Incorrect protection length, dropping FEC packet.";
  ...
  if (fec_packet.protection_length > 0) {
    memcpy(recovered_packet->pkt->data.data() + webrtc::kRtpHeaderSize,
...
bool ForwardErrorCorrection::FinishPacketRecovery(
  ...
  const size_t new_size =
      webrtc::ByteReader<uint16_t>::ReadBigEndian(&data[2]) + webrtc::kRtpHeaderSize;
  if (new_size > size_t{IP_PACKET_SIZE - webrtc::kRtpHeaderSize}) {
...
//                           "typical IP packet, and is thus dropped.";
  ...
  // Set the SN field.
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(&data[2], recovered_packet->seq_num);
  // Set the SSRC field.
  webrtc::ByteWriter<uint32_t>::WriteBigEndian(&data[8], fec_packet.protected_ssrc);
...
void ForwardErrorCorrection::XorHeaders(const Packet& src, Packet* dst) {
  ...
  webrtc::ByteWriter<uint16_t>::WriteBigEndian(src_payload_length_network_order,
                                       src.data.size() - webrtc::kRtpHeaderSize);
...
void ForwardErrorCorrection::XorPayloads(const Packet& src,
                                         ...
                                         Packet* dst) {
  // XOR the payload.
  RTC_DCHECK_LE(webrtc::kRtpHeaderSize + payload_length, src.data.size());
  ...
  for (size_t i = 0; i < payload_length; ++i) {
    dst_data[dst_offset + i] ^= src_data[webrtc::kRtpHeaderSize + i];
  }
}
  ...
  for (const auto& protected_packet : fec_packet.protected_packets) {
    if (protected_packet->pkt == nullptr) {
      ...
      XorPayloads(*protected_packet->pkt,
                  protected_packet->pkt->data.size() - webrtc::kRtpHeaderSize,
                  webrtc::kRtpHeaderSize, recovered_packet->pkt);
    }
  }
