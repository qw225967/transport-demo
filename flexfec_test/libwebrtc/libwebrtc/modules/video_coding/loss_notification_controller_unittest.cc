...
// the mytest, or adding some more specific logic.
class LossNotificationControllerBaseTest : public ::testing::Test,
 ...
 protected:
  ...
  LossNotificationController uut_;  // Unit under mytest.

  bool key_frame_requested_;

  absl::optional<LossNotification> last_loss_notification_;

  // First packet of last frame. (Note that if a mytest skips the first packet
...
TEST_P(LossNotificationControllerTest, LostReferenceProducesLossNotification) {
  ...
  // RTP gap produces loss notification - not the focus of this mytest.
  const bool first = Bool<0>();
  const bool last = Bool<1>();
...
// The difference between this mytest and the previous one, is that in this mytest,
...
TEST_P(LossNotificationControllerTest,
  ...
  // RTP gap produces loss notification - not the focus of this mytest.
  // Also, not decodable; this is important for later in the mytest.
