...
// Generates some simple mytest data in the form of a sawtooth wave.
...
TEST_F(TestVCMJitterEstimator, TestUpperBound) {
  ...
  for (TestContext& context : test_cases) {
    ...
    for (int i = 0; i < 100; ++i) {
      estimator_->UpdateEstimate(gen.Delay(), gen.FrameSize());
      AdvanceClock(time_delta_us);
      estimator_->FrameNacked();      // To mytest rtt_mult.
      estimator_->UpdateRtt(kRttMs);  // To mytest rtt_mult.
